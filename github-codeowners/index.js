...
const ADOPTEDREPOS = process.env.ADOPTEDREPOS;
...
async function processRepoPROpen(repo, filteredTeamsLookup, adoptedRepoTeamsLookup) {
  ...
  if (await getOpenCodeownersPR(repo)) {
    log(`SKIPPING repo ${repo.name} due to open CODEOWNERS Pull Request.`, 'warn');
    return;
  }

  // discover teams to add to CODEOWNERS
  const { ownerTeams, commitMessage } = await generateTeamOwnersForRepo(repo.name, filteredTeamsLookup, adoptedRepoTeamsLookup);
...
async function generateTeamOwnersForRepo(repo, teamsLookup, adoptedRepoTeamsLookup) {
  log(`Generating team ownership for repo: ${repo}`);
  const adoptedRepoTeams = adoptedRepoTeamsLookup[repo];
  const owners = new Set();
  let commitMessage;

  if (adoptedRepoTeams) {
    log(`Found manually adopted teams for ${repo}...`);
    for (const team of adoptedRepoTeams) {
      owners.add(team);
    }
    commitMessage = `Automatically generated by ${USERAGENT}\n\n` +
    'Using manually adopted teams: ' + adoptedRepoTeams.join(', ');
  } else {
  ...
  return { ownerTeams, commitMessage };
}
...
async function createCODEOWNERSBranch(owners, repo, org=ORG, branch=PRBRANCH) {
  if (fs.existsSync(repo)) {
    // ensure we have a clean slate for this work...
    log(`Found dir ${repo} from previous run. cleaning up...`);
...
async function createPullRequest(head, repo, commitMessage, owner=OWNER) {
  log(`Creating pull request for ${repo.name} from ${head} -> ${repo.default_branch}...`);
  const events = 1;
  const perSeconds = 90;
  await waitForSecondaryRateLimitWindow(events, perSeconds);
...
async function waitForSecondaryRateLimitWindow(events=2, seconds=1, sleepMillis=300) {
  ...
  do {
    ...
    if (windowEvents.length > events) {
      if(!isWaiting) {
        log(`Secondary rate-limit guard exceeded. (${windowEvents.length} events/${seconds}secs) Waiting for window to clear...`, 'warn');
        isWaiting=true;
      }
...
async function getOptionalAdoptedRepoLookup() {
  const adoptedRepoLookup = {};
  if (ADOPTEDREPOS) {
    if (fs.existsSync(ADOPTEDREPOS)) {
      try {
        const adoptions = JSON.parse(fs.readFileSync(ADOPTEDREPOS, 'utf8'));
        for (const adoption of adoptions) {
          if (!('repo' in adoption) || !('team' in adoption)) {
            throw new Error('Invalid Format, expected [{repo: "", team: ""}]');
          }
          if (!adoption.repo || !adoption.team) {
            continue;
          }
          adoptedRepoLookup[adoption.repo] = adoption.team.split(' ');
        }
      } catch(err) {
      log(`Couldn't parse JSON file: ${ADOPTEDREPOS}: ${err}`, 'error');
      process.exit(2);
      }
    } else {
      log(`Couldn't find JSON file: ${ADOPTEDREPOS}!`, 'error');
      process.exit(2);
    }
  }
